...
dataf <- function(n,p){
        ...
        df[1:round(n*p,0),1] <- rdist(round(n*p,0))
        df[1:round(n*p,0),2] <- 1
...
sample_power <- function(samples = 100, p, l = 40 , u = 140 , by = 10){
        n <- seq(l,u, by = by)
        
        data <- as.data.frame(matrix(ncol = 2, nrow = length(n), 
                                     c(n, rep(0,length(n)))))
        
        colnames(data) <- c('Sample Size','Power')
